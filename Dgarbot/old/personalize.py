...
def personalize(user_fb_id, user_msg):
    ...
    if current_state == 0:
        new_state, msg = state_0(user_fb_id, user_msg, states)

    elif current_state == 1:
        new_state, msg = state_1(user_fb_id, user_msg, states)

    elif current_state == 2:
        new_state, msg = state_2(user_fb_id, user_msg, states)

    elif current_state == 3:
        new_state, msg = state_3(user_fb_id, user_msg, states)
...
def state_0(user_fb_id, user_msg, states):
    ...
    if user_msg != "guelph" and user_msg != "Guelph":
        msg = "Sorry I do not support the city:" + user_msg + \
...
def state_2(user_fb_id, user_msg, states):
    ...
    #send to new state
    new_state = 3
    msg = "Alright you are all configured! Type anything to get a refresh on the upcoming date or type reset to change any of your settings"
...
def get_current_state(user_fb_id):
    ...
    for user in user_state_list:
        if int(user[0]) == int(user_fb_id):
            current_state = user[1]


    #if the user hasn't been found add it to the list at state 0 and write to db
    if current_state == -1:

        #user id, current state, city, address, week type, pickup day, reminder setting
        temp = [user_fb_id,0,0,0,0,0,False]
        current_state = 0 # Set to 0 also
...
def update_user(entry):
    ...
    for i in range(0,len(user_state_list)):
        if int(user_state_list[i][0]) == int(entry[0]):
